    ...
    for word in word_counter:
        if word[0] in word_translation:
            ...
            if result in word_counter_ch:
                ...
                # save japanese for which can't be tanslated!
                word_counter_ch[result+'/'+word[0]] = word[1]

    lst_words = []
    calculated_at = datetime.now()
    for key, value in word_counter_ch.items():
        if key in list(word_translation.values()):
            word_jp = list(word_translation.keys())[
                list(word_translation.values()).index(key)]
